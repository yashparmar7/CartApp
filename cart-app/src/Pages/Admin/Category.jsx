...
import { RiAddLine, RiEdit2Line, RiDeleteBin6Line, RiStackFill } from "react-icons/ri";
...
<div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-gray-100 shadow-sm mb-8 animate-in fade-in slide-in-from-top-4 duration-500">
  ...
  <div className="flex items-center gap-4">
    ...
    <div className="hidden sm:flex flex-col items-end px-4 border-r border-gray-100">
       <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Active Groups</p>
       <p className="text-sm font-black text-gray-900">{categories?.length || 0} Categories</p>
    </div>
      ...
      <div className="lg:hidden space-y-4">
        {categories.length === 0 && (
          <div className="bg-white rounded-[2rem] p-12 text-center border border-dashed border-gray-200">
            <p className="text-gray-400 font-bold tracking-tight italic">No categories found in system.</p>
          </div>
        )}
          ...
          >
            <div className="flex justify-between items-start mb-4">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-red-50 text-red-500 rounded-xl flex items-center justify-center border border-red-100 shrink-0 shadow-sm">
                   <span className="font-black text-xs uppercase">{cat.name.charAt(0)}</span>
                </div>
                <div>
                  <h3 className="text-sm font-black text-gray-900 uppercase leading-none tracking-tight">{cat.name}</h3>
            ...
            <div className="mt-4 pt-4 border-t border-gray-50 flex items-center justify-between">
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-tighter">
                Added: {new Date(cat.createdAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })}
              </span>
              ...
              {categories.length === 0 && !loading && (
                <tr>
                  <td colSpan="4" className="py-20 text-center bg-gray-50/30">
                    <p className="text-gray-400 font-black uppercase tracking-widest text-sm italic tracking-[0.2em]">Zero Categories Found</p>
                  </td>
                </tr>
              )}
